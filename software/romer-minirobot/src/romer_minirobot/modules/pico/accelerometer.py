...
class Accelerometer(Node):
    ...
    async def tick(self):
        '''
        Measures the acceleration data and in regular intervals

        Returns:
            string: Acceleration data in the form of  (x, y, z)
        '''
        ...
        if time - self.last_time < self.delta_time:
            return
        self.last_time = time
        self.x, self.y, self.z = self.read_accel_data()
        self.set_message(f'({self.x}, {self.y}, {self.z})') 